...
TEST_F(PIDControllerTest, ProportionalTerm) {
    ...
    // P term should be Kp * error = 1.0 * 5.0 = 5.0
    EXPECT_NEAR(output, 5.0f, 0.5f);
}
...
TEST_F(PIDControllerTest, GainAdjustment) {
    pid->setGains(2.0f, 0.5f, 1.0f);
    pid->setTarget(5.0f);
    pid->setCurrent(0.0f);
    float output = pid->compute();
    
    // With higher gains, output should be larger
